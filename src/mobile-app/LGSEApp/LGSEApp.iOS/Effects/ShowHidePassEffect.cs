    ...
    public class ShowHidePassEffect : PlatformEffect
    {
        ...
        private void ConfigureControl()
        {
            if (Control != null)
            {
                ...
                buttonRect.SetImage(new UIImage("eye_open_black_24px"), UIControlState.Normal);
                buttonRect.TouchUpInside += (object sender, EventArgs e1) =>
                {
                    if (vUpdatedEntry.SecureTextEntry)
                    {
                        vUpdatedEntry.SecureTextEntry = false;
                        buttonRect.SetImage(new UIImage("eye_closed_black_24px"), UIControlState.Normal);
                    }
                    else
                    {
