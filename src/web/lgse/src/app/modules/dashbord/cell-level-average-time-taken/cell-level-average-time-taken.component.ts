...
import { CellwiseIncidentChart, CellwiseIncidentResponse, CellwiseIncidentValue } from 'src/app/models/api/dashbord/cellwise-incident-chart';
...
@Component({
  selector: 'app-cell-level-average-time-taken',
  templateUrl: './cell-level-average-time-taken.component.html',
  styleUrls: ['./cell-level-average-time-taken.component.css']
})
export class CellLevelAverageTimeTakenComponent implements OnInit {
  ...
  getCellwiseIncident(): void {
    ...
    this.dashbordService.getCellLevelAvarageTimeTaken(incident).subscribe(
      incidentresponse => {
  ...
  reformatJSONForChart(): void {
    ...
    for (let i = 0; i < this.cellwiseIncidentResponse.length; i++) {
      if (this.cellwiseIncidentResponse[i].series && this.cellwiseIncidentResponse[i].series.length > 0) {
        const data = new CellwiseIncidentChart();
        ...
        for (let j = 0; j < this.cellwiseIncidentResponse[i].series.length; j++) {
          const zonechart = new CellwiseIncidentValue();
          zonechart.name = this.cellwiseIncidentResponse[i].series[j].name;
          // value wants in hour format then use following fucntion.
          // zonechart.value = this.valueFormatting(this.cellwiseIncidentResponse[i].series[j].value);
          if (this.cellwiseIncidentResponse[i].series[j].value > 0) {
            zonechart.value = Math.round(Math.round(this.cellwiseIncidentResponse[i].series[j].value) / 60);
          } else {
            zonechart.value = Math.round(this.cellwiseIncidentResponse[i].series[j].value);
          }
